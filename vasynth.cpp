...
{
	static float b_inSH[VOICES_MAX], b_in[VOICES_MAX], b_f[VOICES_MAX], b_q[VOICES_MAX], b_fp[VOICES_MAX], pole1[VOICES_MAX], pole2[VOICES_MAX], pole3[VOICES_MAX], pole4[VOICES_MAX];
	b_inSH[m] = signal;
	b_in[m] = signal;
...
void VASynth::Process(float *out_l, float *out_r)
{
	...
	for (uint8_t i = 0; i < VOICES_MAX; i++)
	{
		...
		if(vel_select_ == 1 || vel_select_ == 3)
		{
			...
			filter_freq[i] = ((filter_cutoff_ * (env_f_out * velocity[i] * eg_f_amount_) * (1.0f + vcavcflfo_out) * (1.0f - ((vcf_kbd_follow_ * follow[i])))));
		}
		else
		{
			// Disable velocity control of the VCF summed with vcflfo and vcf_kbd_follow
			filter_freq[i] = ((filter_cutoff_ * (env_f_out * eg_f_amount_) * (1.0f + vcavcflfo_out) * (1.0f - ((vcf_kbd_follow_ * follow[i])))));
		}	
		
		filter_out += KarlsenLPF(osc_out, filter_freq[i], filter_res_, i);
	}
